...
from ai import DQNAgent # Import the DQN Agent
...
class Game:
    def __init__(self, layout, ai_agent=None, screen=None, font=None): # Pass screen and font for Pygame
        ...
        self.ai_control = True # Default to AI control
    ...
    def play_ai_turn(self):
        if self.ai_agent and self.game_state == 'PLAYING':
            print(f"\n--- Turn {self.current_turn} (DQN AI) ---")

            # Get current state for DQN
            current_state = self.get_state_for_dqn()
            
            # Get action from DQN agent
            action_idx = self.ai_agent.act(current_state)
    ...
    def get_state_for_dqn(self):
        """
        Returns the current game state in a format suitable for DQN
        """
        from dqn import preprocess_state
        game_state = self.get_state_for_ai()
        return preprocess_state(game_state)

    def get_state_for_ai(self):
        """
        Returns a dictionary representing the current game state for AI processing.
        This is a simplified version that only includes essential information.
        """
        # Create a deep copy of the board to avoid modifying the original
        board_copy = [row[:] for row in self.game_board.grid]
        
        # Convert the board to a more AI-friendly format
        # Replace entities with their symbols
        for row in range(len(board_copy)):
            for col in range(len(board_copy[0])):
                if board_copy[row][col] == 'P':  # Power pellet
                    board_copy[row][col] = 'P'
                elif board_copy[row][col] == '.':  # Regular pellet
                    board_copy[row][col] = '.'
                elif board_copy[row][col] == ' ':  # Empty space
                    board_copy[row][col] = ' '
                elif board_copy[row][col] == 'W':  # Wall
                    board_copy[row][col] = 'W'
    ...
    def _draw_ui(self, surface):
        ...
        control_mode_text = "Manual" if not self.ai_control else "AI"
...
if __name__ == '__main__':
    ...
    for layout_name, layout_config in layouts.items():
        ...
        game = Game(layout_config, ai_agent=None, screen=screen, font=default_font)
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_m: # Still allow manual toggle for quick user intervention if desired
                        game.ai_control = not game.ai_control
                        print(f"Control mode toggled to: {'Manual' if not game.ai_control else 'AI'}")

            # --- Game Logic Update (AI plays by default) ---
            if game.ai_control and not game.is_game_over() and game.ai_agent:
        ...
        if layout_name == "Simple Chase":
            ...
            game = Game(layouts["Simple Chase"], ai_agent=None, screen=screen, font=default_font) # No AI for this part
            game.ai_control = True # Set to manual
